# user-service

## Описание

`user-service` — бэкенд‑сервис управления пользователями и их аутентификацией, реализованный на Go.

Сервис отвечает за:

* учет пользователей системы;
* хранение и управление персональными данными пользователя;
* хранение учетных данных (логин / пароль в зашифрованном виде);
* аутентификацию пользователей и выдачу JWT‑токенов доступа;
* предоставление HTTP API для работы с пользователями.

Сервис предназначен для использования как самостоятельный backend‑компонент и может быть частью монолитного приложения или модульного монолита.

---

## Функциональные возможности

* CRUD‑операции над пользователями
* Аутентификация по логину и паролю
* Выдача JWT access‑token
* Валидация входных данных
* Явное разделение:

  * доменной логики
  * транспортного слоя (HTTP)
  * работы с хранилищем

---

## Архитектура

Проект построен с опорой на идиоматичные практики Go:

* код — основной источник истины;
* HTTP рассматривается как транспорт;
* бизнес‑логика изолирована от деталей хранения и доставки;
* зависимости направлены извне внутрь.

Высокоуровневая структура:

```
user-service/
  ├── api/                    // OpenAPI контракт (временный)
  ├── cmd/
  │   ├── server/             // основной HTTP сервер
  ├── config/                 // файлы конфигураций для разных окружений
  ├── docs/                   // документация mkdocs
  ├── internal/
  │   ├── app/                // сборка и запуск приложения
  │   ├── config/             // конфигурации
  │   ├── auth/               // аутентификация и токены
  │   ├── user/               // домен пользователей
  │   ├── transport/http/     // HTTP handlers, router, middleware
  │   ├── service/            // бизнес-логика (use cases).
  │   ├── repository/         // работа с хранилищем
  │   └── domain/             // чистые структуры и бизнес-типы
  ├── pkg/
  │   └── logger/             // публичный логгер
  ├── migrations/             // миграции для БД
  ├── go.mod
  └── README.md
```

---

## API и OpenAPI

В проекте используется OpenAPI для описания HTTP API.

На текущем этапе:

* спецификация `api/openapi.yaml` поддерживается вручную;
* файл используется как временный контракт.

Планируемый переход:

* OpenAPI будет генерироваться автоматически из Go‑кода на основе аннотаций HTTP‑обработчиков;
* ручная спецификация будет удалена после завершения миграции.

---

## Аутентификация

Сервис реализует базовую аутентификацию пользователей:

* проверка учетных данных (логин / пароль);
* генерация JWT access‑token;
* использование токена для доступа к защищённым эндпоинтам.

JWT и механизмы аутентификации не проникают в доменную модель пользователя и рассматриваются как деталь реализации транспортного слоя.

---

## Технологии

* Go
* net/http
* JWT
* OpenAPI (Swagger)

todo Конкретные реализации хранилищ и инфраструктурных компонентов могут отличаться в зависимости от окружения.

---

## Статус проекта

Проект находится в активной разработке.

API, структура пакетов и внутренняя реализация могут изменяться по мере развития.

---

## Назначение репозитория

Репозиторий предназначен для:

* разработки и развития сервиса управления пользователями;
* использования в качестве backend‑компонента другими приложениями;
* возможного подключения фронтенд‑клиентов (web, admin UI и др.).

## Требования к базе данных

Сервис использует PostgreSQL.

⚠️ Для корректного отображения сообщений об ошибках рекомендуется,
чтобы параметр `lc_messages` был установлен в `Russian_Russia.utf8`.

Подробности см. в [docs/database.md](docs/database.md).

## Флаги запуска
Поддерживаются следующие флаги запуска:

* `--config` - Путь до файла конфигурации, переопределяющий умолчания
* `--version` - Показать информацию о сбоке и выйти
* `--env` - Окружение (development, production, staging)
* `--init-app` - Инициировать приложение и выйти

Примеры:

```bash
go run cmd/server/main.go --env production
go run cmd/server/main.go --config config/production.yaml
go run cmd/server/main.go --version true
go run cmd/server/main.go --init-app true
// Переопределение переменных из команды запуска
DB_PASSWORD=go_user_service JWT_SECRET=test go run cmd/server/main.go --config config/production.yaml
```

## Интеграционные тесты с базой данных

Для проверки корректности работы и инициализации компонента доступа к базе данных в проекте реализованы **интеграционные тесты**, запускаемые с использованием Docker-контейнера PostgreSQL через библиотеку `testcontainers-go`.

### Что проверяется

- Поднятие и доступность PostgreSQL контейнера  
- Корректная инициалиация соединения (`initDB`)  
- Smoke-проверка `Ping` к базе данных

Эти тесты не являются unit-тестами и требуют запуска Docker на вашей машине или в CI-окружении.

### Как это работает

В пакете `internal/app` определён файл `db_integration_test.go`, в котором:

1. Через `TestMain` поднимается контейнер PostgreSQL с жёстко заданной версией (например, `postgres:15.4`).
2. Ожидается готовность сервера базы данных — с ретраями.
3. Создаётся схема (namespace) для тестов.
4. Проверяется, что `initDB` корректно устанавливает соединение.

Очистка данных между отдельными тестами обеспечивается вспомогательной функцией `cleanDatabase`, которая очищает все таблицы текущей схемы.

### Запуск

Чтобы запустить только интеграционные тесты, используйте build tag `integration`:

```bash
go test -tags=integration ./internal/app
```

## CI

Проект использует GitHub Actions для:
- сборки приложения,
- статического анализа кода,
- запуска unit и integration тестов.

Pipeline автоматически запускается:
- при каждом push в ветку `trunk`,
- при открытии pull request в `trunk`.

Для integration-тестов используется PostgreSQL,
запускаемый как service container в GitHub Actions.


Локальная проверка линтером:
Установка версии
```bash
go install github.com/golangci/golangci-lint/cmd/golangci-lint@latest
```

Запуск проверки:
```
golangci-lint run
```

Перед коммитом:
```
go test ./...
golangci-lint run
```

Перед PR (или когда трогал integration-код):
```
golangci-lint run --build-tags=integration
go test -tags=integration ./...
```