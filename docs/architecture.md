...
## Кодогенерация и слои

В проекте используется API-First подход с генерацией кода через `oapi-codegen`. 

### Распределение сгенерированного кода

Весь сгенерированный код (модели запросов/ответов и серверная обвязка) располагается в слое **Transport**:
`internal/transport/http/handler/user/` (пакет `userapi`).

### Взаимодействие со слоями

1. **Transport Layer**: использует сгенерированные DTO (`models_gen.go`) для приема и валидации HTTP-запросов.
2. **Mapping**: Внутри хендлеров данные из DTO копируются в чистые доменные сущности (пакет `internal/domain/user`).
3. **Domain & Service Layers**: **Ничего не знают** о сгенерированном коде. Они работают только с рукописными структурами, что обеспечивает полную изоляцию бизнес-логики от внешних контрактов.
4. **Strict Server**: Мы реализуем **только `StrictServerInterface`**. Базовый `ServerInterface` игнорируется, так как он требует ручной обработки JSON и HTTP-статусов.

**Почему это важно:** Это предотвращает протекание деталей OpenAPI в ядро приложения и избавляет от проблем с циклическими зависимостями при генерации в Go.
